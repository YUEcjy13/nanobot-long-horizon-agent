...
class ContextBuilder:
    ...
    def build_messages(
        ...
        extra: list[str] = []
        goal = parse_goal_state(goal_state_raw(session_metadata)) if session_metadata else None
        if self.enable_goal_runtime_context:
            extra = goal_state_runtime_lines(session_metadata)
        if self.enable_execution_recall:
            recall_lines = self.execution_memory.build_runtime_recall_lines(goal, current_message)
            if recall_lines:
                extra = [*extra, *recall_lines]
