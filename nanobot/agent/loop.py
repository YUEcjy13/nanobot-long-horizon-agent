...
_AUTO_REPLAN_FAILURE_LIMIT = 2
_AUTO_REPLAN_MAX_NOTES = 3
_AUTO_REPLAN_STALE_AFTER_S = 15 * 60
_AUTO_REPLAN_STALL_TURN_LIMIT = 2
...
class AgentLoop:
    ...
    def __init__(
        ...
        self._enable_auto_replan = _env_flag("NANOBOT_ENABLE_AUTO_REPLAN", default=True)
    ...
    @staticmethod
    def _unique_recent_text(existing: list[str], incoming: list[str], *, max_items: int) -> list[str]:
        merged: list[str] = []
        seen: set[str] = set()
    ...
    @staticmethod
    def _auto_replan_failure_notes(tool_events: list[dict[str, str]]) -> list[str]:
        notes: list[str] = []
        for event in tool_events:
            if event.get("status") != "error":
                continue
            name = str(event.get("name") or "tool").strip()
            detail = normalize_goal_text(event.get("detail"), max_chars=220)
            note = f"{name}: {detail}" if detail else f"{name}: tool error"
            notes.append(normalize_goal_text(note, max_chars=300))
        return notes[:_AUTO_REPLAN_MAX_NOTES]

    def _build_auto_replan_injection(
        self,
        goal: dict[str, Any],
        *,
        reasons: list[str],
        ...
        current_step = normalize_goal_text(goal.get("current_step"), max_chars=280) or "(unknown)"
        plan_steps = normalize_goal_list(goal.get("plan_steps"), max_items=5, max_chars=180)
        lines = [
            "[System-generated replanning request]",
            "The active long-running goal hit repeated failure signals.",
            "Do not retry the same failing path immediately.",
            "Before more tool calls, revise the plan with update_goal_state and set mark_replanned=true when the plan changes.",
        ...
        if failure_notes:
            ...
            lines.extend(f"- {note}" for note in failure_notes[:_AUTO_REPLAN_MAX_NOTES])
        if plan_steps:
            lines.append("Current plan snapshot:")
            lines.extend(f"- {step}" for step in plan_steps)
        lines.append(
            "After updating the goal state, continue with the revised step instead of repeating the old failing action."
        )
    ...
    async def _maybe_prepare_legacy_auto_replan_injections(
        ...
        *,
        ...
        reasons = goal_state_replan_reasons(goal, stale_after_s=_AUTO_REPLAN_STALE_AFTER_S)
        if sum(tool_name_counts.values()) >= _AUTO_REPLAN_FAILURE_LIMIT:
            reasons.append("Multiple tool failures occurred before the current step completed.")
        repeated_tools = [name for name, count in tool_name_counts.items() if name and count >= 2]
        if repeated_tools:
            reasons.append(
                "The same tool failed repeatedly in this step: " + ", ".join(sorted(repeated_tools))
            )

        # Progress plateau detection: same current_step across turns without progress.
        current_step = normalize_goal_text(goal.get("current_step"), max_chars=280)
        prev_stalled_step = normalize_goal_text(goal.get("_stalled_step"), max_chars=280)
        prev_completed = len(normalize_goal_list(goal.get("_stalled_completed_snapshot"), max_items=50))
        prev_facts = len(normalize_goal_list(goal.get("_stalled_facts_snapshot"), max_items=20))
        curr_completed = len(normalize_goal_list(goal.get("completed_steps"), max_items=50))
        curr_facts = len(normalize_goal_list(goal.get("verified_facts"), max_items=20))
        stalled_turns = max(0, int(goal.get("_stalled_turn_count") or 0))

        if current_step and current_step == prev_stalled_step and curr_completed <= prev_completed and curr_facts <= prev_facts:
            stalled_turns += 1
        else:
            stalled_turns = 0
        goal["_stalled_turn_count"] = stalled_turns
        goal["_stalled_step"] = current_step
        goal["_stalled_completed_snapshot"] = normalize_goal_list(goal.get("completed_steps"), max_items=50)
        goal["_stalled_facts_snapshot"] = normalize_goal_list(goal.get("verified_facts"), max_items=20)

        if stalled_turns >= _AUTO_REPLAN_STALL_TURN_LIMIT:
            plateau_reason = (
                f"The current step '{current_step}' has not changed for {stalled_turns} consecutive turns "
                f"with no new completed steps or verified facts."
            )
            reasons.append(plateau_reason)

        # Persist plateau tracking state even when injection does not fire,
        # so stalled-turn counts survive across agent turns.
        plateau_state_changed = (
            stalled_turns != max(0, int(goal.get("_stalled_turn_count_prev") or 0))
            or current_step != prev_stalled_step
        )
        if plateau_state_changed:
            ...
            self.sessions.save(session)
        # Snapshot for next turn's comparison.
        goal["_stalled_turn_count_prev"] = stalled_turns

        has_stall_signal = any(("stalled" in reason.lower() or "plateau" in reason.lower()) for reason in reasons)
        should_trigger = bool(reasons) and (
            bool(failure_notes)
            or has_stall_signal
            or stalled_turns >= _AUTO_REPLAN_STALL_TURN_LIMIT
        ) and (
            len(merged_failures) >= _AUTO_REPLAN_FAILURE_LIMIT
            or has_stall_signal
            or bool(repeated_tools)
            or stalled_turns >= _AUTO_REPLAN_STALL_TURN_LIMIT
        )
        ...
        blocked_reason = normalize_goal_text(
            goal.get("blocked_reason"),
            max_chars=800,
        ) or normalize_goal_text(
            f"Repeated failures on current step '{current_step or 'current step'}'; revise the plan before retrying.",
            max_chars=800,
        )
        ...
        failure_summary = blocked_reason
    ...
    async def _run_agent_loop(
        ...
        *on_stream_end(resuming)*: called when a streaming session finishes.
        ...
        async def _after_tools_callback(
            *,
            iteration: int,
            ...
            del iteration, messages, tool_results, fatal_error
            if session is None or not self._enable_auto_replan:
                return []
            return await self._maybe_prepare_auto_replan_injections(
                session,
                tool_calls=tool_calls,
                tool_events=tool_events,
            )
        ...
        try:
            result = await self.runner.run(AgentRunSpec(
                ...
                after_tools_callback=_after_tools_callback,
