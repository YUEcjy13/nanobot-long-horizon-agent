...
class AgentRunSpec:
    ...
    after_tools_callback: Any | None = None
...
class AgentRunner:
    ...
    async def _try_drain_injections(
        ...
        *,
        ...
        prefetched_injections: list[dict[str, Any]] | None = None,
    ) -> tuple[bool, int]:
        ...
        injections = [*(prefetched_injections or []), *(await self._drain_injections(spec))]
        if not injections:
            return False, injection_cycles
        injection_cycles += 1
    ...
    async def _drain_injections(self, spec: AgentRunSpec) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return []
        return self._normalize_injection_items(items)

    @staticmethod
    def _normalize_injection_items(items: list[Any] | None) -> list[dict[str, Any]]:
        if not items:
            return []
    ...
    async def _collect_after_tools_injections(
        self,
        spec: AgentRunSpec,
        *,
        iteration: int,
        ...
        callback = spec.after_tools_callback
        if callback is None:
            return []
        try:
            ...
            logger.exception("after_tools_callback failed")
            return []
        return self._normalize_injection_items(items)
    ...
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        for iteration in range(spec.max_iterations):
            ...
            if response.should_execute_tools:
                ...
                auto_injections = await self._collect_after_tools_injections(
                    spec,
                    iteration=iteration,
                    messages=messages,
                ...
                if fatal_error is not None:
                    ...
                    should_continue, injection_cycles = await self._try_drain_injections(
                        ...
                        prefetched_injections=auto_injections,
                    )
                    if should_continue:
                        had_injections = True
                        continue
