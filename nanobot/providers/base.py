...
def normalize_provider_input(value: str | None, *, field_name: str) -> str | None:
    """Trim common paste artifacts from provider config strings.

    Users sometimes paste API keys or URLs wrapped in smart quotes from IME or
    rich-text sources, which can later break HTTP header encoding.  We strip
    surrounding whitespace and any leading/trailing quote wrappers, while
    leaving the interior untouched.
    """
    if value is None:
        return None
    normalized = value.strip().strip(_PASTED_QUOTE_CHARS)
    if normalized != value:
        logger.warning("Sanitized provider {} by stripping surrounding whitespace/quotes", field_name)
...
class LLMProvider(ABC):
    ...
    def __init__(self, api_key: str | None = None, api_base: str | None = None):
        self.api_key = normalize_provider_input(api_key, field_name="api_key")
        self.api_base = normalize_provider_input(api_base, field_name="api_base")
