...
function buildGoalMarkdownBody(goal: GoalStateWsPayload): string {
  const summary = goal.ui_summary?.trim() ?? "";
  const objective = goal.objective?.trim() ?? "";
  const currentStep = goal.current_step?.trim() ?? "";
  const progressSummary = goal.progress_summary?.trim() ?? "";
  const blockedReason = goal.blocked_reason?.trim() ?? "";
  const planSteps = goal.plan_steps ?? [];
  const completedSteps = goal.completed_steps ?? [];
  const verifiedFacts = goal.verified_facts ?? [];
  const recentFailures = goal.recent_failures ?? [];

  const sections: string[] = [];
  if (summary) sections.push(summary);
  if (objective) sections.push(`### Objective\n${objective}`);
  if (currentStep) sections.push(`### Current Step\n${currentStep}`);
  if (goal.total_steps != null || planSteps.length > 0 || completedSteps.length > 0) {
    const completedCount = goal.completed_count ?? completedSteps.length;
    const totalSteps = goal.total_steps ?? planSteps.length;
    const body: string[] = [];
    if (totalSteps > 0) {
      body.push(`**Plan Progress:** ${completedCount}/${totalSteps} steps completed`);
    }
    if (planSteps.length > 0) {
      body.push("");
      body.push("**Plan Steps**");
      body.push(...planSteps.map((step) => `- ${step}`));
    }
    if (completedSteps.length > 0) {
      body.push("");
      body.push("**Completed Steps**");
      body.push(...completedSteps.map((step) => `- ${step}`));
    }
    sections.push(`### Plan\n${body.join("\n")}`.trim());
  }
  if (progressSummary) sections.push(`### Progress Summary\n${progressSummary}`);
  ...
  if (blockedReason) sections.push(`### Blocked Reason\n${blockedReason}`);
  if ((goal.replan_count ?? 0) > 0 || goal.needs_replan) {
    const notes: string[] = [];
    if ((goal.replan_count ?? 0) > 0) {
      notes.push(`**Replans So Far:** ${goal.replan_count}`);
    }
    if (goal.needs_replan) {
      notes.push("**Replanning Recommended:** yes");
    }
    sections.push(`### Replanning\n${notes.join("\n")}`);
  }
...
function RunElapsedStrip({
  ...
  const markdownBody =
    goalState?.active && (objectiveFull || summaryFull)
      ? buildGoalMarkdownBody(goalState)
      : "";
