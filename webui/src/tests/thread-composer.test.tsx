...
describe("ThreadComposer", () => {
  ...
  it("renders structured goal execution details in the goal panel", async () => {
    render(
      <ThreadComposer
        onSend={vi.fn()}
        placeholder="Type your message..."
        goalState={{
          active: true,
          ui_summary: "Download and summarize paper",
          objective: "Find the paper, download the PDF, and summarize the key method.",
          current_step: "Download the PDF",
          progress_summary: "Found the paper on arXiv and verified the PDF URL.",
          plan_steps: ["Find paper", "Download PDF", "Summarize method"],
          completed_steps: ["Find paper"],
          completed_count: 1,
          total_steps: 3,
          verified_facts: ["The paper is available on arXiv."],
          recent_failures: ["Mirror link returned 403."],
          blocked_reason: "Need a different download source.",
          replan_count: 1,
          needs_replan: true,
        }}
      />,
    ...
    const dialog = await screen.findByRole("dialog", { name: "Goal" });
    expect(dialog).toHaveTextContent("Current Step");
    expect(dialog).toHaveTextContent("Download the PDF");
    expect(dialog).toHaveTextContent("Plan Progress");
    expect(dialog).toHaveTextContent("1/3 steps completed");
    expect(dialog).toHaveTextContent("Verified Facts");
    expect(dialog).toHaveTextContent("The paper is available on arXiv.");
    expect(dialog).toHaveTextContent("Recent Failures");
    expect(dialog).toHaveTextContent("Mirror link returned 403.");
    expect(dialog).toHaveTextContent("Blocked Reason");
    expect(dialog).toHaveTextContent("Replanning Recommended");
  });
