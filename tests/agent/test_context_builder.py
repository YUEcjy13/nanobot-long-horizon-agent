...
class TestBuildMessages:
    ...
    def test_execution_memory_recall_is_injected_for_active_goal(self, tmp_path):
        builder = _builder(tmp_path)
        meta = {
            GOAL_STATE_KEY: {
                "status": "active",
                "goal_id": "goal-123",
                "objective": "Stabilize provider retries.",
                "current_step": "Investigate timeout path",
                "progress_summary": "Need to stop repeated timeout loops.",
            },
        }
        builder.execution_memory.append_episode(
            meta[GOAL_STATE_KEY],
            event_type="failure",
            status="failure",
            step="Investigate timeout path",
            summary="Retrying the same provider call timed out twice.",
            verified_facts=["Timeout happens before fallback provider selection."],
        )

        messages = builder.build_messages(
            [],
            "Please continue debugging the timeout issue.",
            channel="cli",
            chat_id="x",
            session_metadata=meta,
        )
        user_msg = str(messages[-1]["content"])
        assert "Execution Memory Recall:" in user_msg
        assert "timed out twice" in user_msg
        assert "fallback provider selection" in user_msg

    def test_runtime_goal_context_and_execution_recall_can_be_disabled(self, tmp_path, monkeypatch):
        monkeypatch.setenv("NANOBOT_ENABLE_GOAL_RUNTIME_CONTEXT", "0")
        monkeypatch.setenv("NANOBOT_ENABLE_EXECUTION_RECALL", "0")
        builder = _builder(tmp_path)
        meta = {
            GOAL_STATE_KEY: {
                "status": "active",
        ...
        assert "Execution Memory Recall:" not in user_msg
